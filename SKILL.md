...
description: 中文长篇小说全流程创作技能（v1.0.0）。当用户想写小说、创作故事、续写章节、构思剧情、搭建世界观、设计人物关系、提取写作风格、仿写其他小说时，必须使用本技能。覆盖从模糊想法到300万字成品的完整链路：脑洞引导、知识图谱构建、大纲管理、分镜写作、质量门禁、跨Agent审核、风格校准、中途改纲级联更新。即使用户只是说"帮我写个故事"或"我有个小说的想法"，也应触发本技能。
disable-model-invocation: true
---

# Novel Claude AI - 小说创作技能 v1.0.0
...
| `/门禁闭环` | 对当前章节执行六步闭环：更新记忆 → 检查一致性 → 节奏审查 → 风格校准 → 校稿 → 门禁检查 |
...
| 命令 | 功能 | 何时使用 |
|------|------|------|
| `/门禁闭环` | 六步完整门禁：更新记忆→检查一致性→节奏审查→风格校准→校稿→门禁检查 | 每章正文生成后、进入下一章前 |
| `/更新记忆` | 同步状态追踪器 | 每章闭环第1步 |
| `/检查一致性` | 检查剧情/设定/时间线冲突 | 每章闭环第2步 |
| `/节奏审查` | 语义级节奏审查，写入 pacing_review.md（Claude Code 自身执行，无需外部 API） | 每章闭环第3步 |
| `/风格校准` | 检测文风偏移 | 每章闭环第4步 |
| `/校稿` | 去AI味润色 | 每章闭环第5步 |
| `/门禁检查` | 脚本化校验发布标准 | 每章闭环第6步 |
